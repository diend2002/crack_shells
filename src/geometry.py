...
class Geometry:
    ...
    def get_mapping(self,xi):
        if(self.chart == 'hyperb_parab'):
            ...
            radius = 1.
            x = xi[:,0]
            y_temp = (xi[:,1] - np.pi/2.) * radius
            y = radius * torch.sin(y_temp/radius)
            z = radius * torch.cos(y_temp/radius)
            return torch.stack((x,y,z),dim=1)
    ...
    def get_cov_localBasis(self,xi):
        ...
        batch_size, num_channels = self.batch_size, 3
        first_grad_list = []
        for i in range(num_channels):
            v = torch.cat([torch.ones(batch_size,1,device=device)*(i==j) for j in range(num_channels)],1)
        ...
        first_grad_reshape = torch.reshape(first_grad, (batch_size,3,2))
    ...
    def get_cov_localBasis_d(self,xi):
        batch_size, num_channels = self.batch_size, 9
        second_grad_list = []
        for i in range(num_channels):
            v = torch.cat([torch.ones(batch_size,1,device=device)*(i==j) for j in range(num_channels)],1)
