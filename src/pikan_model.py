...
class PyKAN_PINN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.model(x) ## x_NN = NN(xi_1, xi_2, gamma(xi_1,xi_2))
        ...
        if self.bc == 'ls':  # left side clamp 
            ...
            trial_func = (xi[:, 0] + 0.5) * (xi[:, 0] - 0.5)
        elif self.bc == 'fc':
            trial_func = (xi[:, 0]**2 - 0.25) * (xi[:, 1]**2 - 0.25)
        elif self.bc == 'scordelis_lo':
            u[:, 0] = out[:, 0] * (xi[:, 0]**2 + xi[:, 1]**2)
            u[:, 1] = out[:, 1] * (xi[:, 0] + 0.5) * (xi[:, 0] - 0.5)
            u[:, 2] = out[:, 2] * (xi[:, 0] + 0.5) * (xi[:, 0] - 0.5)
            u[:, 3] = out[:, 3]
            u[:, 4] = out[:, 4]
            return u
        elif self.bc == 'fc_circular':
            trial_func = (1. - xi[:, 0] * xi[:, 0] - xi[:, 1] * xi[:, 1]) / 2.
        else:
