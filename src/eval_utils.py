...
def grid_eval_pinn(geometry):
    ...
    arch = [5] 
    bcs = 'fc_circular' # Make sure this matches the saved model's settings
    pn = PyKAN_PINN(2, arch, 5, bcs) 
    pn.load_state_dict(torch.load('models/pn_statedict.pt'))
    pn.eval()
    ...
    np.savetxt('eval/pinn_pred_2.csv', pinn_pred, delimiter=',', header='xi_1,xi_2,u_x,u_y,u_z,th_1,th_2', comments='')
...
def plot_shell(geometry, FEM_sol_dir):
    ...
    data_pinn = np.genfromtxt('eval/pinn_pred_2.csv', delimiter=',', skip_header = 1)
    xi_p = data_pinn[:,0:2]
    ...
    #print("note", color_dimension1, color_dimension2)
    ...
    # # 1. Get the min and max of the FIRST (FEM) dataset
    # # This will define the target range for our color scale.
    # min_fem, max_fem = np.nanmin(color_dimension1), np.nanmax(color_dimension1)

    # # 2. Get the min and max of the SECOND (PINN) dataset
    # min_pinn, max_pinn = np.nanmin(color_dimension2), np.nanmax(color_dimension2)

    # # 3. Scale the PINN data to the FEM data's range
    # # First, normalize the PINN data to the range [0, 1]
    # normalized_pinn = (color_dimension2 - min_pinn) / (max_pinn - min_pinn)
    # # Then, stretch this [0, 1] range to match the [min_fem, max_fem] range
    # scaled_color_dimension2 = normalized_pinn * (max_fem - min_fem) + min_fem

    # # 4. Create the color scale based ONLY on the FEM data's range
    # norm = matplotlib.colors.Normalize(vmin=min_fem, vmax=max_fem)
    # m = plt.cm.ScalarMappable(norm=norm, cmap='inferno')
    # m.set_array([])

    # # 5. Generate colors using the original FEM data and the NEW scaled PINN data
    # fcolors1 = m.to_rgba(color_dimension1)
    # fcolors2 = m.to_rgba(scaled_color_dimension2) # <-- Use the scaled data here
